...
def generate_sort_param():
    ...
    Эта функция создает и возвращает JSON-строку, содержащую начальные параметры сортировки,
    ...
    Возвращает:
        str: JSON-строка, представляющая словарь с параметрами сортировки,
        где ключами являются названия параметров, а значениями - словари
        с параметрами и стилями.
    """
    sort_params = {
        "price": {
            "param": "price",
    ...
    sort_params_json = json.dumps(sort_params)

    return sort_params_json
